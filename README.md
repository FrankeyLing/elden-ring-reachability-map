...
- **非官方粉丝项目**：本工具与 FromSoftware / Bandai Namco 无任何关联，未获其认可或授权。游戏名称、地名、Boss 名、物品名等均为《艾尔登法环》的事实性标识，版权与商标归其权利人所有；地图坐标以事实数据形式呈现，不冒充游戏原始 XYZ，也不担保导航准确性。
- **数据来源与许可**：赐福坐标自行从本地游戏文件副本提取（MSBE 模型 AEG099_060）；地图/关系事实经多个在线来源交叉核对，以事实性名称与关系呈现，不复制任何来源的表达性文本。各来源许可详见 `data/online-source-registry.json`（含 MapForGoblins MIT、eldenpedia CC BY-SA 4.0、er-guide 上游数据 CC BY-NC-SA 3.0 等注记）。
