...
### Source code and release data

Generated large JSON is not source-repository content. `release-data-manifest.json` is the release input manifest; it names the formal graph, acquisition relations, player query projection and pinned online snapshots. Before a release, run:
...
`--check` confirms that Git tracks no JSON larger than 5,242,880 bytes and that all required release inputs exist. The release script copies only Git-tracked code and data explicitly named by the manifest; it never scans or archives local research indexes, original game snapshots or other working-tree artifacts. The archive contains `data/v1/release-manifest.json` with actual file sizes and SHA-256 digests for audit.
